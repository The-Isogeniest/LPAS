...
from ring import vec_l2, pack_ternary, pack_bits, pack_modq, best_group_size, bits_needed  # noqa: E402
...
def phex(p, preview_n: int = 16) -> str:
    # thex() for a single Poly
    return thex([int(x) for x in p.c], preview_n)


def pubhex(v, preview_n: int = 10) -> str:
    ...
    packed = pack_modq(flat, P.Q, MODQ_GROUP)
    preview = [c % P.Q for c in flat[:preview_n]]
    return (f"{_hex_display(packed)}  "
            f"({len(flat)} coeffs packed @ {MODQ_GROUP}/group, "
            f"{8 * len(packed) / len(flat):.1f} bits/coeff -> {len(packed)} bytes; "
            f"first {preview_n} (mod q) = {preview})")


def shorthex(v) -> str:
    # compact lossless hex for a short Gaussian-ish vector, sized to its own max magnitude
    coeffs = flat_coeffs(v)
    bit_width = max(2, bits_needed(max((abs(c) for c in coeffs), default=0)))
    return bhex(coeffs, bit_width)
...
def run_trial(rng: np.random.Generator, verbose: bool) -> dict:
    ...
    try:
        ...
        pke_pk_A1, pke_pk_A2 = req[0]
        out(f"PKE.pk.A1 = {pubhex(pke_pk_A1)}")
        out(f"PKE.pk.A2 = {pubhex(pke_pk_A2)}")
        ...
        pke_c1, pke_c2 = ct_pke
        r = st_advt
        out(f"r (blindness)      = {shorthex(r)}")
        out(f"t = A . r          = {pubhex(t)}")
        out(f"ct.c1 (PKE, part1) = {pubhex(pke_c1)}")
        out(f"ct.c1 (PKE, part2) = {pubhex(pke_c2)}")
        ...
        out(f"z~ (response)   = {shorthex(z_tilde)}   (||z~||_2 = {vec_l2(z_tilde):.1f})")
        out(f"h~ (hint)       = {shorthex(h_tilde)}")
        ...
        out(f"z (full response) = {shorthex(z_sig)}   (||z||_2 = {vec_l2(z_sig):.1f})")
        ...
        out(f"wit' = z - z~  = {shorthex(wit_blinded)}   (||wit'||_2 = {vec_l2(wit_blinded):.1f})")
