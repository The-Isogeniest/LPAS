...
Wires together the other modules:
  ...
  - pke.py        the MLWE-based PKE (Figure 9)
  - ske.py        AES-256-GCM
...
from ring import (
    ...
    scalar_vec_mul, scale_vec, vec_to_bytes, bytes_to_vec, sample_ternary_vec,
    sample_gauss_vec, round_nu, center_mod_vec, combine_rounded,
)
...
def ad_gen(rng, pp: PublicParams, req, wit):
    ...
    for _ in range(P.MAX_REJ_TRIES):
        ...
        c1 = PKE.encrypt(rng, pke_pk, PKE.bytes_to_plaintext_vec(u))
        c2 = SKE.encrypt(k, vec_to_bytes(r))
        ct = (c1, c2)
        t = matvec(pp.A, r)
...
def proxy_pre_verify(pp: PublicParams, msg: bytes, req, advt, presig) -> bool:
    ...
    norm = np.sqrt(vec_l2(z_tilde) ** 2 + vec_l2(h_scaled) ** 2)
...
def req_ext(pp: PublicParams, wit_blinded, st_buyer, stmt, advt):
    ...
    _, _, ct = advt
    c1, c2 = ct
    u = PKE.plaintext_vec_to_bytes(PKE.decrypt(pke_sk, c1), P.LAMBDA // 8)
    k = xor_bytes(u, H_bytes(*[p.tobytes() for p in wit_blinded]))
    try:
        r = bytes_to_vec(SKE.decrypt(k, c2), P.M)
    except Exception:
        return None
